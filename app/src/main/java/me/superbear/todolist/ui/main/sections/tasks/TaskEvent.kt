...
sealed class TaskEvent {
    ...
    data class AddSubtask(val parentId: Long, val title: String, val order: Long? = null) : TaskEvent()
    data class DeleteSubtask(val subtaskId: Long) : TaskEvent()
    data class ToggleSubtask(val childId: Long, val done: Boolean) : TaskEvent()
    data class UpdateSubtaskTitle(val subtaskId: Long, val newTitle: String) : TaskEvent()
